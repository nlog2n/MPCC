...
struct NetAddr    X121_Address;
UCHAR    		  HostIpAddress[4];
...
extern	UCHAR	confbuf[sizeof(pc2card_scb_t)+sizeof(ULONG)];
int	AdapterCount=0;// used in ne2000initialize
VOID MyUnicodeToAnsi(MY_ANSISTRING *mystring,NDIS_STRING *ustring);
...

{
    ...
    *((PUSHORT)(((PEISA_MCA_ADAPTER_IDS)pIds)->IdArray) + 2)=NE2_ADAPTER_ID;
    ...
    NE2000Char.DisableInterruptHandler = NULL;// Ne2000DisableInterrupt;
    NE2000Char.EnableInterruptHandler = NULL;//Ne2000EnableInterrupt;
    ...
    NE2000Char.ResetHandler = NULL;//Ne2000Reset;
...

{
    ...
    NDIS_STRING RamBaseString = RAMBASE;
    ...
    NDIS_STRING X121Address = NDIS_STRING_CONST("X121ADDRESS");
	NDIS_STRING AddressPair = NDIS_STRING_CONST("AddrPair");
    ...
    //ANSI_STRING	AddrBuf,StrBuf;   
    MY_ANSISTRING   AddrBuf;
    ADDR_PAIR_STRING StrBuf;
    ...
    *SelectedMediumIndex = i;
    ...
    RamBaseAddress      = HTDSU_PARAM_RAMBASE1;
    ...
    // Set the MiniPortAdapterHandler first
...
*/
   ...
   NdisReadConfiguration(
	...
	//here read failed,the cause may
	// lie in registry
    ...
    */
	...
	(RamBaseAddress != HTDSU_PARAM_RAMBASE4))
    {
            ...
            ConfigError = TRUE;
            ConfigErrorValue = (ULONG)RamBaseAddress;
            goto RegisterAdapter;
    ...
    ////
    ///set x121 address and addr_table

    

   NdisReadConfiguration(&Status,&ReturnedValue,ConfigHandle,&X121Address,NdisParameterString);
	
   if (Status == NDIS_STATUS_SUCCESS) {
   		unsigned int j;

        MyUnicodeToAnsi(&AddrBuf,&(ReturnedValue->ParameterData.StringData) );
        X121_Address.Len=AddrBuf.Length;
        for (j=0;j<X121_Address.Len;j++)
        	X121_Address.Addr[j]=AddrBuf.Buffer[j];
     
    }


   // Set the address map table 
   NdisReadConfiguration(&Status,&ReturnedValue,ConfigHandle,&AddressPair,NdisParameterString );
	
   if (Status == NDIS_STATUS_SUCCESS) {
        
   		GetAddrPair(&StrBuf,&(ReturnedValue->ParameterData.StringData) );
		InitializeRouteTable(Adapter,&StrBuf);
     }

   
  Cardconf(ConfigHandle, (pc2card_scb_t *)(confbuf+sizeof(ULONG)),(ULONG *)confbuf);
  
   //mainly read parameters
...
RegisterAdapter:
    ...
    //  Now initialize the Packet level.


    PacketLevelInitialize(Adapter);
    //in channel.c ,temporarily
    ...
    Adapter->InstanceNumber=AdapterCount;
	Adapter->Sending    =FALSE;    // No data is transmitting
	AdapterCount++;
...

{
    ...
    */
    ...
    if (status != NDIS_STATUS_SUCCESS){
		DbgPrint("NdisMMapIoSpace Error\n");
        return(status);
	}
    ...
    IF_LOUD(  DbgPrint("Xmit Start (0x%x, 0x%x) : Rcv Start (0x%x, 0x%x) : RcvEnd (0x%x, 0x%x)\n",
              Adapter->XmitStart,
    ...
    status = NdisMRegisterInterrupt(
                 ...
                 Adapter->InterruptNumber,
                 TRUE,// We need a call to the ISR 
                 FALSE,//interrupt unsharable
    ...
    //
    status=CardStart(Adapter);
 	if (status != NDIS_STATUS_SUCCESS)
    {

        
		   DbgPrint("CardState fail\n");
           NdisWriteErrorLogEntry(
            Adapter->MiniportAdapterHandle,
            NDIS_ERROR_CODE_INTERRUPT_CONNECT,
            0
...
--*/
{
    ...
    case OID_GEN_LINK_SPEED:
        ...
        GenericULong = (ULONG)(150);      //100*100bps
